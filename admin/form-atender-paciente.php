  ...
  if (mysqli_multi_query($conexion, $sentencia)) {
    if ($resultado = mysqli_store_result($conexion)) {
      ...
      if ($cuenta == 1) {
        ...
        $id_paciente = $paciente["chr_dni_paciente"];
        ...
        echo json_encode(
          array(
          'existePaciente' => 0,
          'divPaciente' => "<div class='alert alert-danger center-align'><b>Paciente no registrado! Por favor, previamente registre los datos del paciente.</b></div>"."<a href='administrar-filiacion.php' role='button' class='btn btn-primary btn-block' accesskey='r'><i class='fa fa-user'></i> Registrar al paciente (Alt + R)</a>"
          )
        );
      }
    }
